...
<body x-data="{ panelOpen: false }" class="bg-slate-50 text-slate-800 min-h-screen antialiased">
    ...
    <header class="h-20 bg-white/80 backdrop-blur-md border-b border-slate-200 flex items-center justify-between px-8 sticky top-0 z-30">
        ...
        <div class="flex items-center gap-3 shrink-0">
            <div class="w-9 h-9 bg-blue-600 rounded-xl flex items-center justify-center shadow-lg shadow-blue-200 text-white">
                <x-heroicon-o-shield-check class="w-5 h-5"/>
            </div>
            <span class="font-bold text-xl tracking-tight text-slate-900 hidden sm:block">
        ...
        <div class="flex-1 max-w-xl mx-8 relative hidden md:block">
            ...
            <input type="text" placeholder="Cari layanan atau riwayat..." 
        ...
        <div class="flex items-center gap-3 sm:gap-4 ml-auto">
            ...
            <div class="relative" x-data="{ open: false }">
                <button @click="open = !open" class="flex items-center gap-3 text-sm font-medium text-slate-700 hover:text-blue-600 transition p-1 pr-3 rounded-full hover:bg-slate-50 border border-transparent hover:border-slate-100">
                    <div class="w-9 h-9 bg-blue-100 text-blue-600 rounded-full flex items-center justify-center text-xs font-bold border border-blue-200 shadow-sm">
                        {{ substr(Auth::user()->nama ?? 'U', 0, 1) }}
                    </div>
                    <span class="hidden lg:block">{{ Auth::user()->nama ?? 'User' }}</span>
                </button>
                ...
                <div x-show="open" @click.outside="open = false" x-transition x-cloak
                    ...
                    <form action="{{ route('logout') }}" method="POST">
                        @csrf
                        <button type="submit" class="w-full flex items-center gap-2 px-3 py-2.5 text-sm text-red-600 rounded-xl hover:bg-red-50 transition">
    ...
    <div x-cloak x-show="panelOpen" class="fixed inset-0 z-40 overflow-hidden">
        ...
        <div class="absolute inset-y-0 right-0 pl-10 max-w-full flex">
            <aside x-show="panelOpen" x-transition:enter="transform transition ease-in-out duration-300" x-transition:enter-start="translate-x-full" x-transition:enter-end="translate-x-0" x-transition:leave="transform transition ease-in-out duration-300" x-transition:leave-start="translate-x-0" x-transition:leave-end="translate-x-full"
                ...
                <div class="h-20 flex items-center justify-between px-6 border-b border-slate-100 shrink-0">
                    <h2 class="text-lg font-bold text-slate-900">Aktivitas & Sesi</h2>
                ...
                <div class="p-6 overflow-y-auto flex-1 space-y-8">
                    ...
                    <section>
                        <div class="flex items-center justify-between mb-4">
                            <p class="text-[10px] uppercase tracking-wider font-bold text-slate-400">Riwayat Akses</p>
                            <button class="text-[10px] font-bold text-blue-600 hover:underline">Lihat Semua</button>
                        </div>

                        <div class="space-y-6 relative before:absolute before:inset-0 before:ml-3 before:-translate-x-px before:h-full before:w-0.5 before:bg-gradient-to-b before:from-slate-200 before:via-slate-200 before:to-transparent">
                            
                            <div class="relative flex items-center justify-between gap-4 pl-8 group">
                                <div class="absolute left-1.5 w-6 h-6 rounded-full bg-white border-2 border-blue-600 flex items-center justify-center -translate-x-1.5 shadow-sm">
                                    <div class="w-1.5 h-1.5 bg-blue-600 rounded-full"></div>
                                </div>
                                <div class="flex flex-col">
                                    <p class="text-sm font-semibold text-slate-800">Login Sistem</p>
                                    <p class="text-[11px] text-slate-400">Sukses • Chrome Windows</p>
                                </div>
                                <span class="text-[10px] font-medium text-slate-400">Baru saja</span>
                            </div>

                            <div class="relative flex items-center justify-between gap-4 pl-8 group">
                                <div class="absolute left-1.5 w-6 h-6 rounded-full bg-white border-2 border-slate-200 flex items-center justify-center -translate-x-1.5 transition">
                                    <div class="w-1.5 h-1.5 bg-slate-200 rounded-full transition"></div>
                                </div>
                                <div class="flex flex-col">
                                    <p class="text-sm font-semibold text-slate-800">Akses E-Presensi</p>
                                    <p class="text-[11px] text-slate-400">IP: 192.168.1.1</p>
                                </div>
                                <span class="text-[10px] font-medium text-slate-400">10:45 WIB</span>
                            </div>
                        </div>
                    </section>
                </div>
                ...
                <div class="p-6 border-t border-slate-100 bg-slate-50/50">
                    <p class="text-[10px] text-slate-400 text-center italic">Riwayat ini hanya mencakup aktivitas 24 jam terakhir</p>
                </div>
            </aside>
        </div>
    </div>
